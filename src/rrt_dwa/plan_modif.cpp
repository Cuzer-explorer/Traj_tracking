...
int main(int argc, char** argv)
{
    ...
    while(ros::ok()){
        ...
        if (dstPoint_dist < 1.0 && dstPoint_dist > 0.2){  // 减速
            expected_v = ori_sign*1.0 * dstPoint_dist;
        ...
        if (is_forward && abs(dstPoint_angle) > M_PI/2){  // 走过点，停车
            expected_v = 0;
            local_planner.set_v(expected_v);
            expected_w = 0;
        }
        else if(not is_forward && abs(dstPoint_angle) < M_PI/2){
            expected_v = 0;
            local_planner.set_v(expected_v);
            expected_w = 0;
        }
...
void CB_dstPoint(const nav_msgs::Odometry::ConstPtr& odom_msg)  // 目标点
{
    ...
    local_planner.set_v(ori_sign*expected_v);  // DWA 线速度设为定值
